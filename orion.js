...
// ============================================================
// Esta função é essencial para ler corretamente as linhas do CSV
// que contêm aspas duplas e vírgulas dentro dos campos.
// ============================================================

function parseCSVLine(line, linhaNumero) {
    // Array que armazenará todos os campos extraídos
    const valores = [];
    // String que está sendo montada para o campo atual
    let campoAtual = '';
    // Flag que indica se estamos dentro de um campo entre aspas
    let dentroAspas = false;
    // Índice para percorrer a linha caractere por caractere
    let i = 0;

    // Remove espaços e tabs extras no final da linha
    line = line.trimEnd();

    // LOG DE DEPURAÇÃO: Mostra as primeiras 3 linhas para diagnóstico
    if (linhaNumero <= 3) {
        console.log(`   🔎 Linha ${linhaNumero} (início): ${line.substring(0, 150)}...`);
    }
    ...
    while (i < line.length) {
        ...
        if (char === '"') {
            // Caso especial: duas aspas seguidas dentro de um campo
            // Ex: """RUA DR ALVES VALENCA, S N,"""
            // Isso significa que o campo contém uma aspa dupla escapada
            if (dentroAspas && i + 1 < line.length && line[i + 1] === '"') {
                campoAtual += '"';  // Adiciona uma aspa ao campo
                i += 2;             // Pula as duas aspas
                continue;
            } 
            // Abre ou fecha o modo "dentro de aspas"
            else if (dentroAspas) {
                dentroAspas = false; // Fechou as aspas
            } else {
                dentroAspas = true;  // Abriu as aspas
            }
        } 
        // Se encontrou uma vírgula e NÃO está dentro de aspas, é um separador
        else if (char === ',' && !dentroAspas) {
            valores.push(campoAtual); // Salva o campo atual
            campoAtual = '';          // Reseta para o próximo campo
        } 
        // Qualquer outro caractere é adicionado ao campo atual
        else {
            campoAtual += char;
        }
        i++; // Avança para o próximo caractere
    }
    // Adiciona o último campo (depois da última vírgula)
    valores.push(campoAtual);

    // LOG DE DEPURAÇÃO: Mostra quantos campos foram encontrados
    if (linhaNumero <= 3) {
    ...
    // Retorna o array com todos os campos extraídos
    return valores;
}
