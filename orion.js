...
// AUTOR: Eng Souza
// MOTIVO: v5.9.8 — CORREÇÕES CRÍTICAS DE FONTES DE LOCALIZAÇÃO
//         (1) Mozilla Location Service (API descontinuada) — REMOVIDA
//         (2) Bug HTTP/HTTPS no consultarIP — CORRIGIDO (http.get)
//         (3) Cache estendido 24h → 72h — resiste a fins de semana
//         (4) consultarTorreAPIs vai direto ao OpenCellID sem timeout MLS
//         (5) Fluxo WiFi desligado até nova fonte ser integrada
// ============================================================
...
// ============================================================
// PROCESSAMENTO DE LOCALIZAÇÃO — v5.9.8
// Fluxo: GPS → Cache 72h → Unwired → Banco Local → OpenCellID → IP → RSSI → Hermes
// WiFi (Mozilla) + MLS removidos (APIs descontinuadas)
// ============================================================
...
function consultarCache(cellId) { return new Promise((resolve) => { dbCache.get('SELECT lat, lon, range, fonte FROM cell_cache WHERE cell_id = ? AND created_at > datetime("now", "-72 hours")', [cellId], (err, row) => resolve(row ? { cell: cellId, lat: row.lat, lon: row.lon, range: row.range, fonte: row.fonte } : null)); }); }
...
function consultarIP(ip) { return new Promise((resolve) => { dbCache.get('SELECT lat, lon, range FROM ip_cache WHERE ip = ? AND created_at > datetime("now", "-72 hours")', [ip], (err, row) => { if (row) return resolve({ latitude: row.lat, longitude: row.lon, radius: row.range }); const req = http.get('http://ip-api.com/json/' + ip + '?fields=lat,lon', (response) => { let body = ''; response.on('data', chunk => body += chunk); response.on('end', () => { try { const r = JSON.parse(body); if (r.lat && r.lon) { dbCache.run('INSERT OR REPLACE INTO ip_cache (ip, lat, lon, range, created_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)', [ip, r.lat, r.lon, 5000]); resolve({ latitude: r.lat, longitude: r.lon, radius: 5000 }); } else resolve(null); } catch (e) { resolve(null); } }); }); req.on('error', () => resolve(null)); req.setTimeout(3000, () => { req.destroy(); resolve(null); }); }); }); }

async function consultarTorreComRetry(cellId) { for (let i = 0; i < 2; i++) { const r = await consultarOpenCellID(cellId); if (r) return r; if (i < 1) await new Promise(r => setTimeout(r, 1000)); } return null; }
...
function gravarLocalizacao(targetId, cells, wifiData, clientIp, agentId, hermesSession, pos, fonte, res, reqBody) {
    ...
    const cellData = cells ? JSON.stringify(cells.slice(0, 3)) : null;
    const wifiStr = wifiData ? JSON.stringify(wifiData) : null;
    dbMain.run(
        'INSERT INTO locations (target_id, latitude, longitude, radius, source, metodo, torres_usadas, cell_data, wifi_data, ip_data, agent_id, hermes_session, gps_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [targetId, pos ? pos.latitude : null, pos ? pos.longitude : null, pos ? pos.radius : null, fonte, pos && pos.torres_usadas > 1 ? 'triangulacao' : 'dados_brutos', pos ? pos.torres_usadas || cells.length : cells.length, cellData, wifiStr, clientIp, agentId || null, hermesSession || null, gpsData],
        function(err) {
            if (err) return res.status(500).json({ erro: err.message });
            res.json({
                status: pos && pos.latitude ? 'sucesso' : 'falha',
                fonte: fonte,
                position: pos && pos.latitude ? { latitude: pos.latitude, longitude: pos.longitude, raio_estimado: pos.radius, torres_usadas: pos.torres_usadas } : null,
                timestamp: new Date().toISOString()
            });
        }
    );
...
app.listen(port, () => {
    log('info', 'ORION v5.9.8 iniciado na porta ' + port);
    log('info', 'Fontes ativas: Cache (72h), Unwired Labs, Banco Local, OpenCellID, IP, RSSI, Protocolo Hermes');
    log('info', 'Removidas: Mozilla MLS (API descontinuada), WiFi (Mozilla descontinuado)');
});