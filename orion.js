...
CREATE TABLE IF NOT EXISTS locations (id INTEGER PRIMARY KEY AUTOINCREMENT, target_id INTEGER, latitude REAL, longitude REAL, radius INTEGER, source TEXT, metodo TEXT, torres_usadas INTEGER, cell_data TEXT, wifi_data TEXT, ip_data TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`);
...
app.post('/api/localizar-por-cells', (req, res) => {
    ...
    dbMain.get('SELECT id FROM targets WHERE phone = ?', [numero], (err, target) => {
        if (err || !target) { dbMain.run('INSERT INTO targets (name, phone) VALUES (?, ?)', ['Alvo ' + (numero || '').slice(-4), numero], function(insertErr) { if (insertErr) return res.status(500).json({ erro: insertErr.message }); processarLocalizacao(this.lastID, cells, wifiAccessPoints, req.ip, res); }); }
        else { processarLocalizacao(target.id, cells, wifiAccessPoints, req.ip, res); }
    });
});
...
app.post('/api/geolocate', (req, res) => {
    ...
    dbMain.run('INSERT INTO targets (name, phone) VALUES (?, ?)', ['Google API Client', numero], function(insertErr) { processarLocalizacao(this.lastID || 1, cells, wifiAccessPoints, req.ip, res); });
});
...
async function processarLocalizacao(targetId, cells, wifiAccessPoints, clientIp, res) {
    ...
    if (torresEncontradas.length > 0) { gravarLocalizacao(targetId, cells, wifiAccessPoints, clientIp, calcularTriangulacao(torresEncontradas), 'api_externa', res); return; }

    if (clientIp && clientIp !== '127.0.0.1' && clientIp !== '::1') { try { const ipPos = await consultarIP(clientIp); if (ipPos) { gravarLocalizacao(targetId, cells, wifiAccessPoints, clientIp, ipPos, 'ip', res); return; } } catch (e) {} }
